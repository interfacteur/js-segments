...
var common = {
	...
	//Contrôles génériques
		...
		requirePattern: function (e, trig) {
			if (common.require.call(this, e, trig))
...
/* "Objet" comme orientation prototypale ---- ---- ---- ---- ---- ---- ---- ---- */
	...
	Check = function (t) {
		...
		//le formulaire actif :
		this.form = t;
		//le formulaire actif via jQuery :
		this.$form = $(t);
		//action de soumettre :
		this.action = this.$form.data("action");
		//les champs à contrôler - remplacement par data-check ?
		this.$inputs = $(this.$form.find("[required]:not([pattern]), [pattern]:not([required]), [pattern][required]").get().reverse());
		//associer à chaque champ sa méthode de validation et son bloc parent
...
$("body").on("submit", "form", function (e) {
	...
	if (typeof Check.init == "undefined" || Check.init.form != this)
		Check.init = new Check(this);
	Check.init.toCheck();
})
